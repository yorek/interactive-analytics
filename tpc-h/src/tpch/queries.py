...
def query_number(stem: str) -> int:
    return int(stem.split("_")[1])


def query_label(stem: str) -> str:
    return f"Q{query_number(stem)}"
...
def parse_query_filter(args) -> tuple[list[int] | None, int | None]:
    ...
    if args.queries:
        try:
            ids = [int(x) for x in args.queries.split(",") if x.strip()]
        except ValueError:
            print(
                f"Invalid --queries value: {args.queries!r}. "
                "Use comma-separated numbers 1–22, e.g. 2,11,15.",
                file=sys.stderr,
            )
            return None, 2
        for qid in ids:
            if not 1 <= qid <= 22:
                print(f"Query number must be 1–22 (got {qid}).", file=sys.stderr)
                return None, 2
        return ids, None
    return None, None
